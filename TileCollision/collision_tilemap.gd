...
var associated_room : MsdStructs.MSDRoom

func from_msd_room(msd_room : MsdStructs.MSDRoom, screen : int = 0):
	clear()
...
func do_when_clicked(position : Vector2):
	...
	set_cell(0, position, 0, Vector2i(Globals.active_collision_tile_index%16, floor(Globals.active_collision_tile_index/16)))
	var position_to_save = position + top_left_offset
