...
var current_selected_layer : TreeItem = null
...
func select_layer(layer : TreeItem):
	if current_selected_layer:
		current_selected_layer.clear_custom_bg_color(0)
	
	current_selected_layer = layer
	
	current_selected_layer.set_custom_bg_color(0, Color.RED)
	if current_selected_layer.get_metadata(0) is TileMap:
		activated.emit(current_selected_layer)
